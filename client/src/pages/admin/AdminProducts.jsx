...
function Toast({ message, type, onDone }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      gsap.to(ref.current, {
        y: 24, opacity: 0, duration: 0.2, onComplete: onDone,
      });
    }, 2800);
      ...
      {value ? (
        <>
          ...
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
            <p className="text-white text-xs font-medium">Tap to replace</p>
          </div>
        </>
      ) : (
        <>
          <ImagePlus size={22} className="text-white/20 mb-1.5" />
...
function CategoryModal({ onSave, onClose }) {
  ...
  const close = () => {
    ...
    gsap.to(modalRef.current, { y: 16, opacity: 0, duration: 0.18, onComplete: onClose });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const created = await apiFetch("/categories", {
        method: "POST",
        body: JSON.stringify({ name: name.trim(), slug: slug || autoSlug(name) }),
      });
      onSave(created);
      close();
    } catch (e) {
...
function ProductModal({ product, categories, onSave, onClose, onCategoryCreate }) {
  ...
  const close = () => {
    gsap.to(overlayRef.current, { opacity: 0, duration: 0.18 });
    gsap.to(modalRef.current, { y: 20, opacity: 0, duration: 0.18, onComplete: onClose });
  };
        ...
        >
          ...
          <div className="px-5 py-4 space-y-4 max-h-[65vh] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Price ($)" icon={DollarSign} error={errors.price}>
                <InputField
                  type="number"
                  placeholder="0.00"
                  value={form.price}
      ...
      <div ref={boxRef} className="w-full max-w-sm bg-[#1e2436] rounded-2xl border border-white/8 p-5 shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-white/35 mt-0.5">This can't be undone.</p>
          </div>
        </div>
...
function ProductCard({ product, categories, onEdit, onDelete, index }) {
  ...
  useEffect(() => {
    gsap.fromTo(
      ref.current,
      { opacity: 0, y: 16 },
      { opacity: 1, y: 0, duration: 0.3, ease: "power2.out", delay: index * 0.05 }
    );
  }, []);
      ...
      <div className="flex-1 px-3 py-3 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-2">
          <div className="flex items-baseline gap-1">
            ...
            {product.discount > 0 && (
              <span className="text-[11px] text-white/30 line-through">{formatPrice(product.price)}</span>
            )}
          </div>
          {product.rating && (
            <div className="flex items-center gap-1">
              <Star size={10} className="text-amber-400 fill-amber-400" />
              <span className="text-[11px] text-white/50">{product.rating}</span>
            </div>
          )}
          <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ml-auto ${stockColor}`}>
...
function ProductRow({ product, categories, onEdit, onDelete, index }) {
  ...
  useEffect(() => {
    gsap.fromTo(
      rowRef.current,
      { opacity: 0, x: -10 },
      { opacity: 1, x: 0, duration: 0.28, ease: "power2.out", delay: index * 0.04 }
    );
  }, []);
  ...
  return (
    ...
    >
      <td className="py-3 px-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {product.description && (
              <p className="text-xs text-white/30 truncate max-w-[160px]">{product.description}</p>
            )}
          </div>
        </div>
      </td>
      ...
      <td className="py-3 px-4">
        <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${stockColor}`}>
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Total products" value={products.length} icon={Package} color="bg-[#6c63ff]" />
        <StatCard label="Categories" value={categories.length} icon={Tag} color="bg-[#3b82f6]" />
        <StatCard label="Out of stock" value={outOfStock} icon={AlertTriangle} color="bg-[#ff6584]" />
        <StatCard label="Catalog value" value={formatPrice(totalValue)} icon={DollarSign} color="bg-emerald-500" />
      </div>

      {/* Filters */}
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-[140px] h-10 px-3 rounded-xl bg-[#1e2436] border border-white/8 focus-within:border-[#6c63ff]/50 transition-colors">
                ...
                <thead>
                  <tr className="border-b border-white/5">
                    {["Product", "Category", "Price", "Stock", "Rating", ""].map((h) => (
                      <th key={h} className="text-left py-3 px-4 text-[11px] font-semibold text-white/25 uppercase tracking-wider">
                        {h}
                      </th>
                    ))}
                  </tr>
