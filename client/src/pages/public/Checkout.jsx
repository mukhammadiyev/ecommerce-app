...
// ── Sample cart data ──
const CART_ITEMS = [
  { id: 1, name: "Double Bed & Dressing", qty: 1, price: 180 },
  { id: 2, name: "Double Bed & Dressing", qty: 1, price: 120 },
  { id: 3, name: "Double Bed & Dressing", qty: 1, price: 100 },
];
const SHIPPING = 100;
        ...
        return (
          ...
          >
            <span
              ...
              ${state === "active" ? "bg-white text-gray-900" : state === "done" ? "bg-gray-400 text-white" : "bg-gray-200 text-gray-400"}`}
            >
              {state === "done" ? (
...
// ── Billing form ── saved cards + optional new card ──
...
export default function Checkout() {
  ...
  const validateBilling = () => {
    if (!useNewCard) return true; // saved card, no extra validation
    const errs = {};
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-8 sm:py-10">
      ...
      {showModal && (
        <ThankYouModal
          ...
          onCheckOrder={() => {
            setShowModal(false); /* navigate to order details */
          }}
        />
      )}
        ...
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-7 items-start">
          ...
          <div className="w-full lg:flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 sm:px-6 pt-6 pb-6">
              ...
              {/* Actions */}
              <div
                className={`flex items-center mt-7 ${step > 0 ? "justify-between" : "justify-start"}`}
              >
                {step > 0 && (
                  <button
                    onClick={goBack}
                    className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-700 transition-colors duration-150"
                  >
                ...
                <button
                  onClick={goNext}
                  className="flex items-center gap-2 bg-gray-900 text-white text-sm font-semibold px-6 py-3 rounded-full hover:bg-gray-700 active:scale-[0.97] transition-all duration-200"
                >
                  {ctaLabel}
          ...
          <div className="w-full lg:w-80 xl:w-84 shrink-0">
            <CartSummary items={CART_ITEMS} shipping={SHIPPING} />
          </div>
        </div>
      </div>
    </div>
